...
const OrderCompleteToggleButton = ({ order }) => {
	...
	const {mutate} = useMutation({
		...
		onSuccess: async () => { 
			...
			setIsLoading(false); 
			navigate(`/orders?view=${order.completed ? 'inprogress' : 'completed'}`)},
	});
	...
	return isLoading ? 
		...
		<Tooltip title={order.completed ? "Change to 'In Progress'" : "Mark as Completed"}>
			<Button sx={{ color: order.completed ? "green" : "grey" }} onClick={handleSubmit}>
